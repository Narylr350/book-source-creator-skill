...
社区版目标需要已安装并启动社区维护版阅读 App、开启 `MCP service`。电脑与设备处于可互访的局域网时，可直接连接 `http://<设备IP>:1236/mcp`，不需要 adb；局域网不可达或不方便使用设备 IP 时，也可以通过 adb 临时转发连接。AI 会读取目标 App 当前提供的 JavaScript 契约，生成脚本后在 App 内完成保存、读回、四链路调试和校验。
