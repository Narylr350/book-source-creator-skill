...
- **匿名分析可以继续，最终验收不能只靠匿名。** 只要站点有 `loginUrl` / `enabledCookieJar` / `Authorization` / `webJs` / `webView` 任一项，最终验证优先登录态。匿名生成结果只能标 `anonymous_candidate` 或高风险，不能标 full pass。
- 用户拒绝登录时可以继续生成，但最终状态必须是 `degraded` / `high-risk`，需要 App 复核。
...
**CSR/WebView 边界**：遇到正文可能是 CSR/WebView 时，优先用 `mode=android` 跑 Probe 验证；无 Android 设备时跑 `mode=http` 确认 CSR 空壳后标 `validator_limitation`。不能只跑 `http` 就下结论。

验证分流：

- **passed** → 若无登录态特征（loginUrl/enabledCookieJar/Authorization/webJs/webView），进入交付；若命中登录态特征，匿名 passed 只能算 `anonymous_candidate`，需登录态验证后才能交付
- **anonymous_candidate** → 匿名验证通过，但站点存在登录态/WebView/Cookie 依赖，不能标可用，需登录态/App 复核
- **failed** → 读 `validator-report.json` 中的 `error`、`ruleHits`、`phases`，AI 自动回修规则，再跑 validator（最多 5 次）
- **needs_app_review** → 停止自动修，标记需 App/浏览器复核
- **validator_limitation** → 标记工具缺口，不误判站点不可用
- **failed_unresolved** → 5 次回修后仍未通过，标记未解决
