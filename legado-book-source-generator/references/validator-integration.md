...
| `CONTENT_IS_VIP_LOCK_PAGE` | 正文页提示 VIP/付费 | WARNING，收敛为 `needs_app_review`，不能标 full pass |
...
- VIP/付费页：可能只是当前账号没有订阅/付费权限；记录为 `content:vip` 警告并收敛到 `needs_app_review`，不能宣称 VIP 支持或 full pass。
