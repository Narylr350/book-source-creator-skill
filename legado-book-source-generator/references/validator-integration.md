...
- `bookUrl`（可选）：跳过搜索，直接从指定书籍 URL 开始验证 detail/toc/content。对齐阅读 App debug 模式的 URL 直接入口（`key.isAbsUrl()`）。搜索被反爬阻塞时用此参数继续后续链路。
...
| `CAPTCHA_DETECTED` | 搜索返回验证码/人机验证页。validator 等价于阅读 App，App 也会弹验证码。不是规则错误。 | — |
...
| `CONTENT_IS_VIP_LOCK_PAGE` | 正文页提示 VIP/付费 | WARNING，收敛为 `degraded`，不能标 full pass |
...
| Code | 说明 |
|------|------|
| `APP_REVIEW_REQUIRED` | validator 无法分类的兜底（验证码/反爬现在走 `CAPTCHA_DETECTED`/`CONTENT_IS_CAPTCHA_PAGE`，不走此 code） |
...
elif step.errorCode == CAPTCHA_DETECTED (search/detail/toc):
    status = "blocked"  // bsg 返回 blockedBy: "captcha"，提示登录或 --book-url
elif step.errorCode == CONTENT_IS_CAPTCHA_PAGE:
    status = "failed"  // 正文验证码，不是 needs_app_review
