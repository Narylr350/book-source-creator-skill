...
启动: `node scripts/bsg.mjs validator-start`。停止: `node scripts/bsg.mjs validator-stop`。
...
```bash
node scripts/bsg.mjs validate --run runs/<slug> [--mode android]
node scripts/bsg.mjs record-validation --run runs/<slug> --status <status>
```
...
- 安装并启动 Probe：运行 `node scripts/bsg.mjs login`
- `bsg.mjs login` 是唯一入口：检测 adb、必要时安装 adb、安装 APK、启动 Probe、配置端口转发并检查 `http://127.0.0.1:18888/ping`
...
```text
advance（进入 validate）→ bsg.mjs validate --run runs/<slug> → record-validation → advance（进入 deliver）
```
