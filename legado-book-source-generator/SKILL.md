...
**拿到 URL 后第一件事就是运行 `init`。不先探测、不先分析、不先翻源码。**

```bash
node "<skill-dir>/scripts/bsg.mjs" init <site-url> [--cwd <输出目录>]
```

`init` 返回 `nextAction: "probe_site"`。然后按 `nextAction` 执行每个阶段，每阶段完成后运行 `advance` 进入下一阶段：
...
**禁止跳过 init。禁止在 init 之前做任何探测。禁止手动创建 runs/ 目录或 run-state.json。**
