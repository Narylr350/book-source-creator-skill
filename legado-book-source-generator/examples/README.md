...
- `webview-fallback-site/`: WebView回退样例（正文有签名，使用WebView模式）
...
- 当目标站点因为签名、加密、CSR 空壳或浏览器渲染而接近被判成 `不建议生成` 时，必须先回看样例与 `references/reference-source-patterns.md`，确认是否存在可复用的 fallback 模式，例如 WebView 模式。
