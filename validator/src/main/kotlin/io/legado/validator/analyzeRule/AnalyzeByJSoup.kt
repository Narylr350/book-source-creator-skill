...
class AnalyzeByJSoup(doc: Any) {
    ...
    private fun getResultList(ruleStr: String): ArrayList<String>? {
        ...
        // 先用 getElementsSingle 解析所有选择器（与 getElements 行为一致）
        ...
        // 最后一个规则：如果是已知关键字(text/html等)则提取内容，否则当作选择器再解析一轮
        val lastRule = rules.last()
        if (lastRule !in listOf("text", "textNodes", "ownText", "html", "all")) {
            // 不是已知关键字，当作 CSS 选择器解析
            val es = Elements()
            for (elt in elements) {
                es.addAll(ElementsSingle().getElementsSingle(elt, lastRule))
            }
            if (es.isNotEmpty()) {
                elements = es
                return getResultLast(elements, "text")
            }
        }
        return if (elements.isEmpty()) null else getResultLast(elements, lastRule)
    }
        ...
        fun getElementsSingle(temp: Element, rule: String): Elements {
            ...
            if (indexes.isEmpty()) for (ix in lastIndexes downTo 0) {
                ...
                if (indexes[ix] is Triple<*, *, *>) {
                    ...
                    val start = if (startX == null) 0
                    else if (startX >= 0) if (startX < len) startX else len - 1
                    else if (-startX <= len) len + startX else 0
                    val end = if (endX == null) len - 1
                    else if (endX >= 0) if (endX < len) endX else len - 1
                    else if (-endX <= len) len + endX else 0
